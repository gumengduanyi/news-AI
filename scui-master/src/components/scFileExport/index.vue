		...
		watch:{
			'formData.fileType'(val) {
				if(this.formData.fileName.includes(".")){
					this.formData.fileName = (this.formData.fileName && typeof this.formData.fileName === 'string') ? this.formData.fileName.substring(0, this.formData.fileName.lastIndexOf('.')) + "." + val : ''
				}else{
