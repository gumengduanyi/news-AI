	...
	export default {
		...
		methods: {
			...
			_isImg(fileUrl){
				...
				const fileExt = (fileUrl && typeof fileUrl === 'string') ? fileUrl.substring(fileUrl.lastIndexOf(".")) : ''
				return imgExt.indexOf(fileExt) != -1
			},
			_getExt(fileUrl){
				return (fileUrl && typeof fileUrl === 'string') ? fileUrl.substring(fileUrl.lastIndexOf(".") + 1) : ''
			}
		}
	}
</script>
